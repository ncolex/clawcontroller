...
async def startup_event():
    print("BACKEND_STARTUP_VERIFIED_V3")
...
class BotWAConfig(BaseModel):
    mode: str
    whitelist: List[str]
    blacklist: List[str]
    allowGroups: bool
    adminNumbers: List[str]
    autoReply: bool
    welcomeMessage: str
        ...
        while True:
            ...
            try:
                ...
                if event == "agent_soul_updated":
                    ...
                    # Create a mock session manager for backend context
                    class BackendSessionManager:
                        def get_session(self, agent_id):
                            # In backend context, sessions are managed by OpenClaw
                            # This is a passthrough for OpenClaw gateway integration
                            return None
...
@app.get("/api/agents/{agent_id}/soul", response_model=AgentSoulResponse)
def get_agent_soul(agent_id: str, db: Session = Depends(get_db)):
    """
    Get agent SOUL.md content for hot-reload in OpenClaw.

    This endpoint is called by OpenClaw when it receives an 'agent_soul_updated'
    WebSocket event. It returns the current SOUL.md for the specified agent.
    ...
    Returns:
        AgentSoulResponse with agent_id and soul_markdown fields

    Raises:
        HTTPException: 404 if agent not found, 500 if OpenClaw config missing
    """
    home = Path.home()
    config_path = home / ".openclaw" / "openclaw.json"

    # Try to get SOUL from OpenClaw config first
    if config_path.exists():
        ...
        agent_config = next((a for a in agent_list if a.get("id") == agent_id), None)
        ...
        if agent_config:
            ...
            agent_dir = Path(
                agent_config.get(
                    "agentDir",
                    home / ".openclaw" / f"workspace-{agent_id}"
                )
            )

            # Fallback to old workspace structure
            ...
            if soul_path.exists():
                return AgentSoulResponse(
                    agent_id=agent_id,
                    soul_markdown=soul_path.read_text()
                )

    # Fallback: try database agent
    agent = db.query(Agent).filter(Agent.id == agent_id).first()
    if agent:
        return AgentSoulResponse(
            agent_id=agent_id,
            soul_markdown=agent.description or ""
        )
...
async def save_agent_soul(
    agent_id: str,
    request: SaveSoulRequest,
    db: Session = Depends(get_db)
):
    """
    Save agent SOUL.md content and broadcast hot-reload event to OpenClaw.

    This endpoint:
    1. Saves the new SOUL.md to the agent's workspace
    2. Broadcasts 'agent_soul_updated' WebSocket event to connected clients
    3. OpenClaw gateway receives the event and hot-reloads the session

    Args:
        agent_id: The agent identifier
        request: SaveSoulRequest with soul_markdown field
        db: Database session
    ...
    # Try to save to OpenClaw workspace first
    if config_path.exists():
        try:
            with open(config_path) as f:
                config = json.load(f)
        ...
        if agent_config:
            ...
            # Write SOUL.md
            soul_path = agent_dir / "SOUL.md"
            soul_path.write_text(request.soul_markdown)

            # Broadcast hot-reload event to OpenClaw
            await manager.broadcast({
                "event": "agent_soul_updated",
                "agent_id": agent_id,
                "score": 100  # Default score for manual updates
            })
    ...
    # Fallback: update database agent
    agent = db.query(Agent).filter(Agent.id == agent_id).first()
    if agent:
        agent.description = request.soul_markdown
        db.commit()
...
def get_openclaw_agents(db: Session = Depends(get_db)):
    ...
    for agent in agent_list:
        ...
        # Normalize model to string (handle both string and dict formats)
        if isinstance(agent_model, dict):
            agent_model = agent_model.get("primary") or agent_model.get("model") or str(agent_model)
...
async def delete_task(task_id: str, db: Session = Depends(get_db)):
    ...
    try:
        ...
        # 1. Delete TaskActivity records
        db.query(TaskActivity).filter(TaskActivity.task_id == task_id).delete()
        
        # 2. Delete ActivityLog entries
        db.query(ActivityLog).filter(ActivityLog.task_id == task_id).delete()
        
        # 3. Delete RecurringTaskRun entries (IMPORTANT: this was missing)
        db.query(RecurringTaskRun).filter(RecurringTaskRun.task_id == task_id).delete()
        
        # 4. Delete the task itself (Comments and Deliverables are handled by SQLAlchemy cascade)
        db.delete(task)
        db.commit()
        
        # Broadcast the deletion via WebSocket so the frontend updates in real-time
        await manager.broadcast({"type": "task_deleted", "data": {"id": task_id}})
        # Also broadcast tasks_updated for general refresh
        await manager.broadcast({"type": "tasks_updated", "data": {}})
...
# ============ AI & Control Enhancements ============

class OptimizePromptRequest(BaseModel):
    text: str

@app.post("/api/ai/optimize-soul")
async def optimize_soul(data: OptimizePromptRequest):
    """Optimize a simple natural language instruction into a professional OpenClaw SOUL.md."""
    # Usamos tu propia API en Render que ya sabemos que funciona
    RENDER_URL = "https://smollm-mini.onrender.com/generate"
    instruction_text = f"Convert this simple instruction into a professional, highly detailed, and effective 'SOUL.md' for an AI agent. Use Markdown format. Be direct, define personality, goals, and constraints. Instruction: {data.text}"
    ...
    try:
        ...
        if res.status_code == 200:
            return {"optimized_text": res.json().get('text', '')}
        else:
            raise HTTPException(status_code=500, detail=f"Render API Error: {res.status_code}")
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

@app.post("/api/agents/{agent_id}/stop")
async def stop_agent(agent_id: str, db: Session = Depends(get_db)):
    """Stop/Kill an agent and its sessions if stuck."""
    try:
        # 1. Try to kill local processes related to this agent
        subprocess.run(["pkill", "-f", f"agent.*{agent_id}"], check=False)
        
        # 2. Try to clean OpenClaw sessions for this agent
        subprocess.run(["openclaw", "sessions", "clear", "--agent", agent_id], check=False)
        
        # 3. Update status in database to STANDBY
        agent = db.query(Agent).filter(Agent.id == agent_id).first()
        if agent:
            agent.status = AgentStatus.STANDBY
            db.commit()
            await manager.broadcast({"type": "agent_status", "data": {"id": agent_id, "status": "STANDBY"}})
            
        return {"ok": True, "message": f"Agent {agent_id} stopped and sessions cleared"}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_ollama_status():
    """Get Ollama local status and running models."""
    ...
    try:
        with urllib.request.urlopen('http://localhost:11434/api/tags', timeout=5) as resp:
            if resp.status == 200:
                ...
                # Categorize models
                local_models = [m for m in models if not m.get('remote_host')]
                remote_models = [m for m in models if m.get('remote_host')]
                
                # Calculate total size
                total_size_gb = sum(m.get('size', 0) for m in local_models) / (1024**3)
                    ...
                    "models": [
                        {
                            ...
                            "is_remote": bool(m.get('remote_host'))
                        }
                        for m in models
                    ]
                }
...
# Subagents status endpoint
@app.get("/api/subagents/status")
async def get_subagents_status(db: Session = Depends(get_db)):
    """Get real-time status of subagents/spawned sessions."""
    import subprocess
    import json
    ...
    try:
        ...
        sessions_data = json.loads(result.stdout) if result.stdout else {"sessions": []}
        
        # Get active tasks to correlate with subagents
        active_tasks = db.query(Task).filter(
            Task.status.in_(["ASSIGNED", "IN_PROGRESS", "REVIEW"])
        ).all()
        
        # Build subagents list from sessions
        subagents = []
        for session in sessions_data.get("sessions", []):
            key = session.get("key", "")
            updated_at = session.get("updatedAt", 0)
            
            # Parse session key: agent:{agent_id}:{source}:{id}
            parts = key.split(":")
            ...
            import time
            time_since_update = time.time() - (updated_at / 1000) if updated_at else 999
            is_active = time_since_update < 60  # Active if updated in last 60 seconds
            
            subagents.append({
                "id": f"sub_{agent_id}_{source}",
                "parent_agent": agent_id,
                "source": source,
                "session_key": key,
                "is_active": is_active,
                "last_activity": updated_at,
                "time_since_update_seconds": round(time_since_update, 1),
                "current_task": associated_task,
        ...
        return {
            ...
            "active_subagents": len([s for s in subagents if s["is_active"]]),
            "subagents": subagents
        }
...
# Backup and System Status endpoint
@app.get("/api/system/backup-status")
async def get_backup_status():
    """Get backup and system startup status."""
    import subprocess
    import os
    from datetime import datetime
    ...
    try:
        ...
        def get_service_info(service_name):
            try:
                ...
                return result.stdout[:500] if result.stdout else "No info"
            except:
                return "Error getting status"
        
        # Check if lingering is enabled
        def check_lingering(username):
            try:
                result = subprocess.run(
        ...
        # Check Ollama status
        ollama_active = check_service("ollama")
        ollama_pid = None
        try:
            result = subprocess.run(
            ...
            if result.stdout.strip():
                ollama_pid = result.stdout.strip().split("\n")[0]
        except:
            pass
        
        # Check ClawController services
        clawcontroller_active = check_service("clawcontroller")
        clawcontroller_pid = None
        try:
            result = subprocess.run(
                ["pgrep", "-f", "uvicorn.*clawcontroller"],
                capture_output=True,
                text=True,
                timeout=5
            )
            if result.stdout.strip():
                clawcontroller_pid = result.stdout.strip().split("\n")[0]
        except:
            pass
        
        # Check Vercel deployment
        vercel_status = "deployed"  # Assume deployed if we're running
        
        # Get current user
        username = os.getenv("USER", "ncx")
        lingering_enabled = check_lingering(username)
        
        # Check for backup file
        backup_config_path = "/home/ncx/.clawcontroller/BACKUP_CONFIG.json"
        ...
        return {
            ...
            "services": {
                "ollama": {
                    ...
                    "pid": ollama_pid,
                    "auto_start": check_service("ollama"),
                    "status": "running" if ollama_active else "stopped"
                },
                "clawcontroller": {
                    "active": clawcontroller_active,
                    "pid": clawcontroller_pid,
                    "auto_start": check_service("clawcontroller"),
                    "status": "running" if clawcontroller_active else "stopped"
                }
            },
            "systemd": {
                "lingering_enabled": lingering_enabled,
                "username": username
            },
            "backup": {
                "google_drive": {
                    "configured": backup_exists,
                    "folder": "AI_Projects_Backup",
                    "document": "OpenClaw & ClawController Master Config",
                    "last_backup": backup_data.get("last_backup") if backup_data else None
                },
                "config_file": backup_config_path
            },
            "deployment": {
                "vercel": {
                    "status": vercel_status,
                    "url": "https://clawcontroller.vercel.app"
                }
            },
...
async def delete_recurring_task(recurring_id: str, db: Session = Depends(get_db)):
    """Delete a recurring task and all its associated data (tasks, activities, runs)."""
    rt = db.query(RecurringTask).filter(RecurringTask.id == recurring_id).first()
    if not rt:
        raise HTTPException(status_code=404, detail="Recurring task not found")
    
    try:
        # 1. Get all task IDs spawned from this recurring task
        runs = db.query(RecurringTaskRun).filter(RecurringTaskRun.recurring_task_id == recurring_id).all()
        spawned_task_ids = [run.task_id for run in runs if run.task_id]
        
        # 2. For each task, delete all its dependencies manually to satisfy constraints
        for t_id in spawned_task_ids:
            db.query(TaskActivity).filter(TaskActivity.task_id == t_id).delete()
            db.query(ActivityLog).filter(ActivityLog.task_id == t_id).delete()
            # Comments and Deliverables should cascade delete via SQLAlchemy
            db.query(Task).filter(Task.id == t_id).delete()
            
            # Broadcast deletion for each task
            await manager.broadcast({"type": "task_deleted", "data": {"id": t_id}})
        
        # 3. Delete all run records
        db.query(RecurringTaskRun).filter(RecurringTaskRun.recurring_task_id == recurring_id).delete()
        
        # 4. Finally delete the recurring task template
        db.delete(rt)
        db.commit()
        
        # 5. Broadcast removal of the recurring task
        await manager.broadcast({"type": "recurring_deleted", "data": {"id": recurring_id}})
        
        return {"ok": True}
    except Exception as e:
        db.rollback()
        print(f"Error deleting recurring task {recurring_id}: {str(e)}")
        ...
        if run.task_id:
            ...
            if task_obj:
                task = {
                    ...
                    "completed_at": task_obj.updated_at if task_obj.status == TaskStatus.COMPLETE else None
                }
        result.append({
            "id": run.id,
            "run_at": run.run_at,
...
# BotWA Configuration Endpoints
BOTWA_CONFIG_PATH = Path("/home/ncx/.openclaw/agents/henry/workspace/botwa/backend/config/botConfig.json")

@app.get("/api/botwa/config")
async def get_botwa_config():
    if not BOTWA_CONFIG_PATH.exists():
        # Fallback to an empty config if not found
        return {
            "mode": "whitelist",
            "whitelist": [],
            "blacklist": [],
    ...
    try:
        with open(BOTWA_CONFIG_PATH, "r") as f:
            return json.load(f)
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

@app.post("/api/botwa/config")
async def save_botwa_config(config: BotWAConfig):
    try:
        # Ensure the directory exists
        BOTWA_CONFIG_PATH.parent.mkdir(parents=True, exist_ok=True)
        with open(BOTWA_CONFIG_PATH, "w") as f:
