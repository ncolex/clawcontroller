...
export const useMissionStore = create((set, get) => ({
  ...
  // BotWA Panel
  openBotWAPanel: () => set({ isBotWAPanelOpen: true }),
  closeBotWAPanel: () => set({ isBotWAPanelOpen: false }),
  toggleBotWAPanel: () => set((state) => ({ isBotWAPanelOpen: !state.isBotWAPanelOpen })),
  ...
  // ============ BotWA Actions ============
  fetchBotWAConfig: async () => {
    try {
      const config = await api.fetchBotWAConfig()
      return config
    } catch (error) {
      console.error('Failed to fetch BotWA config:', error)
      throw error
    }
  },

  saveBotWAConfig: async (config) => {
    try {
      await api.saveBotWAConfig(config)
    } catch (error) {
      console.error('Failed to save BotWA config:', error)
      throw error
    }
  },
